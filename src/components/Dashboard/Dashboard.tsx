...
import { BarChart3, TrendingUp, Users, Star, Library, Settings as SettingsIcon, Shield, User as UserIcon } from 'lucide-react';
...
export function Dashboard({
    ...
    const loadUsers = async () => {
      try {
        ...
        if (api?.admin?.listUsers) {
          ...
          if (res?.ok) {
            const userList = (res.users || []).map((u: any) => {
            ...
            userList.forEach(async (user: any) => {
              if (user.email) {
                try {
                  const url = await avatarService.getAvatarUrl(user.email, true);
                  if (url) {
  ...
  // Handle favorite toggle with new service
  const handleToggleFavorite = async (resourceId: string) => {
    ...
    try {
      ...
      if (success) {
        // Call the parent's toggle favorite handler
  ...
  const categoryData = categories.map(cat => ({
    id: (cat as any).id || String((cat as any).name || 'unknown'),
    name: (cat as any).name || 'Unnamed Category',
  ...
  const favoriteResources = resources.filter(r => userFavorites.includes(r.id)).slice(0, 6);
    ...
    <div className="p-6 md:p-8 space-y-6 overflow-y-auto scrollbar-hide" style={{scrollbarWidth: 'none', msOverflowStyle: 'none'}}>
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-gray-900">
          ...
          <div className="space-y-2">
            {categoryData.slice(0, 4).map((cat, i) => {
              ...
              return (
                <div key={String((cat as any).id ?? `${cat.name}-${i}`)} className="flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-gray-900">
          ...
          <div className="text-2xl font-semibold text-gray-900 dark:text-white">{resources.filter(r => userFavorites.includes(r.id)).length}</div>
        </div>
        <div className="rounded-xl border border-gray-200 dark:border-gray-800 p-4 bg-white dark:bg-gray-900">
          <div className="flex items-center gap-2 mb-2">
            <BarChart3 className="h-5 w-5 text-emerald-500" />
            ...
            {favoriteResources.slice(0, 4).map(resource => {
              ...
              return (
                <div key={resource.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors" onClick={() => onOpenResource(resource)}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleToggleFavorite(resource.id); }} className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700">
            ...
            {recentActivity.slice(0, 4).map(resource => {
              ...
              return (
                <div key={resource.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer transition-colors" onClick={() => onOpenResource(resource)}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); handleToggleFavorite(resource.id); }} className="p-1 rounded hover:bg-gray-200 dark:hover:bg-gray-700">
                    <Star className={`h-4 w-4 ${userFavorites.includes(resource.id) ? 'text-yellow-500 fill-current' : 'text-gray-400'}`} />
                  </button>
                </div>
              );
            })}
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {users.slice(0, 6).map(usr => {
              ...
              const displayRole = isMe && profile?.accountType ? String(profile.accountType) : String((usr as any).role || 'Free');
              return (
                  ...
                  {displayRole === 'CEO' ? (
                    <div className="relative w-16 h-16 mx-auto mb-2 flex-shrink-0">
                      ...
                      <div className="absolute inset-[2px] rounded-full overflow-hidden bg-gradient-to-br from-blue-500 to-purple-600 flex items-center justify-center">
                        {userAvatars[usr.id] ? (
                          <img src={userAvatars[usr.id]} alt={usr.name} className="w-full h-full object-cover rounded-full" />
                        ) : usr.avatarUrlMeta ? (
                          <img src={usr.avatarUrlMeta} alt={usr.name} className="w-full h-full object-cover rounded-full" />
                        ) : (
                    ...
                    <div className="relative w-16 h-16 mx-auto mb-2 flex-shrink-0">
                      {userAvatars[usr.id] ? (
                        <img src={userAvatars[usr.id]} alt={usr.name} className={`w-full h-full object-cover rounded-full ring-2 ring-offset-1 dark:ring-offset-gray-950 ring-offset-white ${displayRole === 'Admin' ? 'ring-amber-500' : displayRole === 'Premium' ? 'ring-emerald-500' : 'ring-gray-400'}`} />
                      ) : usr.avatarUrlMeta ? (
                        <img src={usr.avatarUrlMeta} alt={usr.name} className={`w-full h-full object-cover rounded-full ring-2 ring-offset-1 dark:ring-offset-gray-950 ring-offset-white ${displayRole === 'Admin' ? 'ring-amber-500' : displayRole === 'Premium' ? 'ring-emerald-500' : 'ring-gray-400'}`} />
                      ) : (
