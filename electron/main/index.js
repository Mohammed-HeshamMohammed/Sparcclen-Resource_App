  ...
  "node_modules/@supabase/storage-js/dist/main/lib/helpers.js"(exports2) {
    ...
    var resolveResponse = () => __awaiter(void 0, void 0, void 0, function* () {
      if (typeof Response === "undefined") {
        return (yield Promise.resolve(`${"@supabase/node-fetch"}`).then((s) => __importStar(require(s)))).Response;
      }
      return Response;
    });
        ...
        return new Promise((resolve3, reject) => {
          fetcher(url, _getRequestParams(method, options, parameters, body)).then((result) => {
            ...
            if (options === null || options === void 0 ? void 0 : options.noResolveJson)
              return result;
            return result.json();
          }).then((data) => resolve3(data)).catch((error) => handleError(error, reject, options));
        });
      });
    }
...
// electron/main/security/credentialManager.ts
...
var CredentialManager = class {
  ...
  CRED_FILE;
  constructor() {
    this.CRED_DIR = (0, import_path.join)(import_electron.app.getPath("documents"), "Sparcclen");
    this.CRED_FILE = (0, import_path.join)(this.CRED_DIR, ".credentials.enc");
  }
  async ensureDir() {
    if (!(0, import_fs2.existsSync)(this.CRED_DIR)) {
      (0, import_fs2.mkdirSync)(this.CRED_DIR, { recursive: true });
    }
  }
  /**
  ...
  async promptWindowsHello(email) {
    ...
    try {
      const result = await import_electron.dialog.showMessageBox({
        type: "info",
...
Your Windows login already protects these credentials via Windows Data Protection API (DPAPI).`,
  ...
  async storeCredentials(email, password) {
    try {
      ...
      const encryptedPassword = import_electron.safeStorage.encryptString(password);
      let credentials = {};
      if ((0, import_fs2.existsSync)(this.CRED_FILE)) {
        const data = await import_fs.promises.readFile(this.CRED_FILE, "utf-8");
        credentials = JSON.parse(data);
      }
      credentials[email] = encryptedPassword.toString("base64");
      await import_fs.promises.writeFile(this.CRED_FILE, JSON.stringify(credentials, null, 2), "utf-8");
      return true;
    } catch (error) {
  ...
  async getCredentials(email) {
    try {
      ...
      return import_electron.safeStorage.decryptString(encryptedBuffer);
    } catch (error) {
...
// electron/main/state/uploadTracker.ts
var UploadTracker = class {
  pending = 0;
  begin() {
    this.pending += 1;
    return this.pending;
...
// electron/main/ipc/windowControls.ts
var import_electron2 = require("electron");
var registerWindowControlHandlers = (getWindow, uploads) => {
  import_electron2.ipcMain.handle("win:minimize", () => {
    const window2 = getWindow();
    if (!window2) return false;
    window2.minimize();
    return true;
  });
  import_electron2.ipcMain.handle("win:maximize", () => {
    const window2 = getWindow();
    if (!window2) return false;
  ...
  import_electron2.ipcMain.handle("uploads:begin", () => uploads.begin());
  import_electron2.ipcMain.handle("uploads:end", () => uploads.end());
  import_electron2.ipcMain.handle("win:close", () => {
    const window2 = getWindow();
    if (!window2) return false;
  ...
  import_electron2.ipcMain.handle("win:getSize", () => {
    const window2 = getWindow();
    const [width, height] = window2?.getSize() ?? [0, 0];
    return { width, height };
  });
};

// electron/main/ipc/saveHandlers.ts
var import_electron4 = require("electron");

// electron/main/persistence/saveStore.ts
var import_electron3 = require("electron");
var import_path2 = require("path");
var import_fs3 = require("fs");
var import_fs4 = require("fs");
var SAVE_DIR = (0, import_path2.join)(import_electron3.app.getPath("documents"), "Sparcclen");
var SAVE_PATH = (0, import_path2.join)(SAVE_DIR, "DID-Data.save");
var defaultSave = {
  firstRun: true,
  theme: "system",
...
var ensureSaveDir = async () => {
  if (!(0, import_fs3.existsSync)(SAVE_DIR)) {
    (0, import_fs3.mkdirSync)(SAVE_DIR, { recursive: true });
  }
};
var readSaveFile = async () => {
  await ensureSaveDir();
  try {
    const txt = await import_fs4.promises.readFile(SAVE_PATH, "utf-8");
    const data = JSON.parse(txt);
    return { ...defaultSave, ...data };
  } catch {
    await import_fs4.promises.writeFile(SAVE_PATH, JSON.stringify(defaultSave, null, 2), "utf-8");
    return { ...defaultSave };
  }
};
var writeSaveFile = async (patch) => {
  const current = await readSaveFile();
  const next = { ...current, ...patch, updatedAt: (/* @__PURE__ */ new Date()).toISOString() };
  await import_fs4.promises.writeFile(SAVE_PATH, JSON.stringify(next, null, 2), "utf-8");
  return next;
};
var ensureInitialSaveFile = async () => {
  await readSaveFile();
};

// electron/main/ipc/saveHandlers.ts
var registerSaveHandlers = () => {
  import_electron4.ipcMain.handle("save:read", async () => readSaveFile());
  import_electron4.ipcMain.handle("save:write", async (_event, patch) => {
    return writeSaveFile(patch ?? {});
  });
};

// electron/main/ipc/credentialHandlers.ts
var import_electron5 = require("electron");
var registerCredentialHandlers = (manager) => {
  import_electron5.ipcMain.handle("credentials:isAvailable", () => manager.isEncryptionAvailable());
  import_electron5.ipcMain.handle(
    "credentials:store",
    async (_event, email, password) => manager.storeCredentials(email, password)
  );
  import_electron5.ipcMain.handle(
    "credentials:get",
    async (_event, email) => manager.getCredentials(email)
  );
  import_electron5.ipcMain.handle(
    "credentials:getEmails",
    async () => manager.getStoredEmails()
  );
  import_electron5.ipcMain.handle(
    "credentials:has",
    async (_event, email) => manager.hasCredentials(email)
  );
  import_electron5.ipcMain.handle(
    "credentials:delete",
    async (_event, email) => manager.deleteCredentials(email)
  );
  import_electron5.ipcMain.handle(
    "credentials:promptHello",
    async (_event, email) => manager.promptWindowsHello(email)
  );
};

// electron/main/ipc/fileSystemHandlers.ts
var import_electron6 = require("electron");
var import_fs5 = require("fs");
var import_path3 = require("path");
var resolveDocumentsPath = (relativePath) => {
  const documentsPath = import_electron6.app.getPath("documents");
  let adjustedPath = relativePath;
  if (/^Documents[\\/](?:Sparcclen)[\\/]/.test(adjustedPath)) {
    adjustedPath = adjustedPath.replace(/^Documents[\\/](Sparcclen)[\\/]/, (_, base) => `${base}/`);
  } else if (!/^Sparcclen[\\/]/.test(adjustedPath)) {
    adjustedPath = `Sparcclen/${adjustedPath}`;
  }
  return (0, import_path3.resolve)(documentsPath, adjustedPath);
};
var registerFileSystemHandlers = () => {
  import_electron6.ipcMain.handle("fs:writeFile", async (_event, relativePath, data) => {
    try {
      const fullPath = resolveDocumentsPath(relativePath);
      await import_fs5.promises.writeFile(fullPath, data, "utf-8");
      return true;
    } catch (error) {
  ...
  import_electron6.ipcMain.handle("fs:readFile", async (_event, relativePath) => {
    try {
      const fullPath = resolveDocumentsPath(relativePath);
      return await import_fs5.promises.readFile(fullPath, "utf-8");
    } catch (error) {
      console.error("[fs:readFile] Error:", error);
      return null;
    }
  });
  import_electron6.ipcMain.handle("fs:exists", async (_event, relativePath) => {
    try {
      const fullPath = resolveDocumentsPath(relativePath);
      await import_fs5.promises.access(fullPath);
      return true;
    } catch {
      return false;
    }
  });
  import_electron6.ipcMain.handle("fs:ensureDir", async (_event, relativePath) => {
    try {
      const fullPath = resolveDocumentsPath(relativePath);
      await import_fs5.promises.mkdir(fullPath, { recursive: true });
      return true;
    } catch (error) {
...
// electron/main/ipc/resourceHandlers.ts
var import_electron8 = require("electron");
var import_path5 = require("path");
var import_fs6 = require("fs");

// electron/main/utils/paths.ts
var import_electron7 = require("electron");
var import_path4 = require("path");
var sanitizePathSegment = (segment) => segment.replace(/[:*?"<>|]/g, "").replace(/[/\\]+/g, "").trim();
var resolveLibraryBaseDir = () => {
  if (process.platform === "win32") {
    const localAppData = process.env["LOCALAPPDATA"];
    if (localAppData && localAppData.trim()) {
      return (0, import_path4.join)(localAppData, "Sparcclen");
    }
  }
  return (0, import_path4.join)(import_electron7.app.getPath("userData"), "Sparcclen");
};

// electron/main/ipc/resourceHandlers.ts
var registerResourceHandlers = (getWindow) => {
  import_electron8.ipcMain.handle("resources:pickJsonFile", async () => {
    try {
      const targetWindow = getWindow() ?? void 0;
      const result = await import_electron8.dialog.showOpenDialog(targetWindow, {
      ...
      const data = await import_fs6.promises.readFile(filePath, "utf-8");
  ...
  import_electron8.ipcMain.handle(
    ...
    async (_event, segments, fileName, content) => {
      try {
        const sanitizedSegments = Array.isArray(segments) ? segments.map(sanitizePathSegment).filter(Boolean) : [];
        const safeFileName = sanitizePathSegment(fileName) || "library.bin";
        const baseDir = resolveLibraryBaseDir();
        const targetDir = (0, import_path5.join)(baseDir, ...sanitizedSegments);
        await import_fs6.promises.mkdir(targetDir, { recursive: true });
        const fullPath = (0, import_path5.join)(targetDir, safeFileName);
        await import_fs6.promises.writeFile(fullPath, Buffer.from(content, "utf-8"));
  ...
  import_electron8.ipcMain.handle(
    ...
    async (_event, options) => {
      try {
        const baseDir = resolveLibraryBaseDir();
        const libraryRoot = (0, import_path5.join)(baseDir, "library");
        const results = [];
        const libraryExists = await import_fs6.promises.stat(libraryRoot).then((stat) => stat.isDirectory()).catch(() => false);
        if (!libraryExists) {
          return { ok: true, files: results };
        }
        const categoryFilter = options?.category ? sanitizePathSegment(options.category) : null;
        const subcategoryFilter = options?.subcategory ? sanitizePathSegment(options.subcategory) : null;
        const collectBins = async (targetDir, categorySegment, subcategorySegment) => {
          try {
            const entries = await import_fs6.promises.readdir(targetDir, { withFileTypes: true });
            for (const entry of entries) {
              if (!entry.isFile()) continue;
              if (!entry.name.toLowerCase().endsWith(".bin")) continue;
              const filePath = (0, import_path5.join)(targetDir, entry.name);
              try {
                const raw = await import_fs6.promises.readFile(filePath, "utf-8");
        ...
        const processCategoryDirectory = async (dirName) => {
          const sanitizedName = sanitizePathSegment(dirName);
          if (!sanitizedName) return;
          if (categoryFilter && sanitizedName.toLowerCase() !== categoryFilter.toLowerCase()) {
            return;
          }
          const categoryDir = (0, import_path5.join)(libraryRoot, dirName);
          if (!subcategoryFilter) {
            await collectBins(categoryDir, dirName, null);
          }
          const entries = await import_fs6.promises.readdir(categoryDir, { withFileTypes: true }).catch(() => []);
          for (const entry of entries) {
            if (!entry.isDirectory()) continue;
            const subDirName = entry.name;
            const sanitizedSub = sanitizePathSegment(subDirName);
            if (!sanitizedSub) continue;
            if (subcategoryFilter && sanitizedSub.toLowerCase() !== subcategoryFilter.toLowerCase()) {
              continue;
            }
            await collectBins(
              (0, import_path5.join)(categoryDir, subDirName),
              dirName,
              subDirName
            );
          }
        };
        const topLevelEntries = await import_fs6.promises.readdir(libraryRoot, { withFileTypes: true }).catch(() => []);
        if (!categoryFilter) {
  ...
  import_electron8.ipcMain.handle(
    ...
    async (_event, sourceDir, imagePath) => {
      try {
        const cleanImagePath = imagePath.startsWith("./") ? imagePath.slice(2) : imagePath;
        const sourcePath = (0, import_path5.join)(sourceDir, cleanImagePath);
        const sourceExists = await import_fs6.promises.stat(sourcePath).then(() => true).catch(() => false);
        if (!sourceExists) {
          return {
            ok: false,
        ...
        const imageBuffer = await import_fs6.promises.readFile(sourcePath);
  ...
  import_electron8.ipcMain.handle(
    ...
    async (_event, url) => {
      try {
        await import_electron8.shell.openExternal(url);
      } catch (error) {
...
// electron/main/ipc/adminHandlers.ts
var import_electron9 = require("electron");
var import_supabase_js = __toESM(require_main6());
var getAdminClient = () => {
  const url = "https://ldklvotqnvpluhmshfjm.supabase.co";
  const key = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imxka2x2b3RxbnZwbHVobXNoZmptIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1OTYwNjk5MywiZXhwIjoyMDc1MTgyOTkzfQ.0MKQtZH2OEF_VPCp6YvZHTVv2cmUpmpJ8WV7Zbg0HyM";
  if (!url || !key) return null;
  try {
    return (0, import_supabase_js.createClient)(url, key, { auth: { persistSession: false, autoRefreshToken: false } });
  } catch (error) {
    console.warn("[admin] failed to create client", error);
    return null;
  }
};
var registerAdminHandlers = () => {
  import_electron9.ipcMain.handle("admin:listUsers", async () => {
  ...
  import_electron9.ipcMain.handle("admin:updateUserRole", async (_event, userId, role) => {
    const admin = getAdminClient();
    if (!admin) return { ok: false, error: "Admin API not configured" };
    try {
...
// electron/main/ipc/viewsFavsHandlers.ts
var import_electron11 = require("electron");

// electron/main/persistence/viewsFavsStore.ts
var import_electron10 = require("electron");
var import_path6 = require("path");
var import_fs7 = require("fs");
var DIR = (0, import_path6.join)(import_electron10.app.getPath("documents"), "Sparcclen");
var FILE = (0, import_path6.join)(DIR, "views_favs.enc");
async function ensureDir() {
  await import_fs7.promises.mkdir(DIR, { recursive: true }).catch(() => {
  });
}
async function saveViewsFavs(items) {
  try {
    await ensureDir();
    const payload = JSON.stringify(items);
    if (import_electron10.safeStorage.isEncryptionAvailable()) {
      const enc = import_electron10.safeStorage.encryptString(payload);
      await import_fs7.promises.writeFile(FILE, enc);
    } else {
      await import_fs7.promises.writeFile(FILE, payload, "utf-8");
    }
    return true;
  } catch (err) {
...
async function loadViewsFavs() {
  try {
    const buf = await import_fs7.promises.readFile(FILE);
    if (!buf || buf.length === 0) return [];
    if (import_electron10.safeStorage.isEncryptionAvailable()) {
      try {
        const clear = import_electron10.safeStorage.decryptString(Buffer.from(buf));
        const parsed2 = JSON.parse(clear);
        return Array.isArray(parsed2) ? parsed2 : [];
      } catch {
        const txt2 = buf.toString("utf-8");
...
// electron/main/ipc/viewsFavsHandlers.ts
var registerViewsFavsHandlers = () => {
  import_electron11.ipcMain.handle("vf:load", async () => {
    return await loadViewsFavs();
  });
  import_electron11.ipcMain.handle("vf:save", async (_event, items) => {
...
var resolvePreloadPath = () => {
  const prodPreload = (0, import_path7.join)(__dirname, "..", "preload.js");
  const devPreloadBuilt = (0, import_path7.join)(__dirname, "..", "preload", "index.js");
  const devPreloadRegister = (0, import_path7.join)(__dirname, "..", "preload", "dev-register.cjs");
  return [prodPreload, devPreloadBuilt, devPreloadRegister].find((p) => (0, import_fs8.existsSync)(p));
};
var createMainWindow = () => {
  ...
  const getIconPath = () => {
    const possiblePaths = [
      ...
      (0, import_path7.join)(__dirname, "..", "..", "src", "public", "Logo.ico"),
      (0, import_path7.join)(__dirname, "..", "..", "..", "src", "public", "Logo.ico"),
      // Fallback to .png if .ico doesn't exist
      (0, import_path7.join)(__dirname, "..", "..", "src", "public", "Logo.png"),
      (0, import_path7.join)(__dirname, "..", "..", "..", "src", "public", "Logo.png"),
      // Final fallback to root level
      (0, import_path7.join)(__dirname, "..", "..", "Logo.ico"),
      (0, import_path7.join)(__dirname, "..", "..", "Logo.png")
    ];
  ...
  const mainWindow2 = new import_electron12.BrowserWindow({
    width: 900,
    height: 750,
    minWidth: 850,
    minHeight: 700,
  ...
  mainWindow2.webContents.setWindowOpenHandler((details) => {
    import_electron12.shell.openExternal(details.url);
    return { action: "deny" };
  });
  mainWindow2.on("resize", () => {
    if (mainWindow2.isDestroyed()) return;
    const [width, height] = mainWindow2.getSize();
    mainWindow2.webContents.send("win:resize", { width, height });
  });
  if (is.dev) {
    const devUrl = process.env["VITE_DEV_SERVER_URL"] || "http://127.0.0.1:5175";
    mainWindow2.loadURL(devUrl).catch((error) => {
    ...
    mainWindow2.loadFile((0, import_path7.join)(__dirname, "..", "..", "Releases", "index.html"));
  }
  return mainWindow2;
};
...
(0, import_dotenv.config)({ path: (0, import_path8.resolve)(process.cwd(), ".env") });
...
var setupEnvironment = () => {
  if (is2.dev) {
    const devUserData = (0, import_path8.join)(import_electron13.app.getPath("appData"), "SparcclenDev");
    import_electron13.app.setPath("cache", (0, import_path8.join)(devUserData, "Cache"));
    process.env.ELECTRON_DISABLE_SECURITY_WARNINGS = "true";
  }
};
var setupAppIcon = () => {
  const appId = is2.dev ? "com.sparcclen.dev" : "com.sparcclen.app";
  import_electron13.app.setAppUserModelId(appId);
};
var setupSingleInstanceLock = () => {
  const gotTheLock = import_electron13.app.requestSingleInstanceLock();
  if (!gotTheLock) {
    import_electron13.app.quit();
    return false;
  }
  import_electron13.app.on("second-instance", () => {
    const existing = import_electron13.BrowserWindow.getAllWindows()[0];
    if (existing) {
